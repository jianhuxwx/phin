...
export class CacheRepository {
  ...
  async listRecentBlocks(limit: number): Promise<CachedBlockSummary[]> {
    const values = await this.redis.lrange(RECENT_BLOCKS_KEY, 0, Math.max(limit - 1, 0));
    return values
  ...
  async getBlockById(id: string): Promise<CachedBlockDetail | null> {
    return safeParse<CachedBlockDetail>(await this.redis.get(`block:${id}`));
  }

  async getBlockByHeight(height: number): Promise<CachedBlockDetail | null> {
    return safeParse<CachedBlockDetail>(await this.redis.get(`block:height:${height}`));
  }

  async getTransaction(id: string): Promise<any | null> {
    return safeParse<any>(await this.redis.get(`tx:${id}`));
  }

  async getNetworkStats(): Promise<CachedNetworkStats | null> {
    return safeParse<CachedNetworkStats>(await this.redis.get('network:stats'));
  }

  async getGatewayStatuses(): Promise<CachedGatewayStatus[]> {
    const value = await this.redis.get('network:gateways');
